# -*- coding: utf-8 -*-
'''
Experiment on Robustness to noise for filter pruning using Frame theory
Authors: Pradyumna, Pradip, Ramunaidu
'''
...
def test(epoch, testloader):
    ...
    if acc > best_acc:
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        torch.save(state, './checkpoint/ETF_Noisy/{}-{}-{}-ckpt.t7'.format(args.net, args.dataset, args.noise))
        best_acc = acc

    os.makedirs("log", exist_ok=True)
    content = time.ctime() + ' ' + f'Epoch {epoch}, lr: {optimizer.param_groups[0]["lr"]:.7f}, val loss: {test_loss:.5f}, acc: {(acc):.5f}'
    print(content)
    log_file = f'log/log_{args.net}_{args.dataset}_noise{args.noise}.txt'
...
if __name__ == '__main__':
    ...
    # Noise data
    trainset_noise = dataset_class(root='./data', train=True, download=True, transform=transform_train_noise)
    trainloader_noise = torch.utils.data.DataLoader(
        trainset_noise, batch_size=bs, shuffle=True, num_workers=8
    )

    testset_noise = dataset_class(root='./data', train=False, download=True, transform=transform_test_noise)
    testloader_noise = torch.utils.data.DataLoader(
        testset_noise, batch_size=100, shuffle=False, num_workers=8
    )
    ...
    pacc_1, pacc_5 = calculate_accuracy(net, testloader_noise, device)
    print(f'UnPruned Accuracy on Noisy Data:{pacc_1:.2f}%')
    ...
    flops, params = get_model_complexity_info(
        net, input_size, as_strings=True, print_per_layer_stat=True
    )
    print(f"FLOPs before pruning: {flops}")
    print(f"Parameters before pruning: {params}")

    print("Pruning started")
    prune_per = torch.tensor([0.25] * 5 + [0.65] * 8)
    our_pruned_model_ae(net, prune_per)

    flops_a, params_a = get_model_complexity_info(
        net, input_size, as_strings=True, print_per_layer_stat=False
    )
    ...
    for epoch in range(start_epoch, args.n_epochs):
        ...
        trainloss = train_kd(
            epoch, trainloader, teacher=teacher, student=net,
            T=4.0, alpha=0.5
        )

        val_loss, acc = test(epoch, testloader_noise)
